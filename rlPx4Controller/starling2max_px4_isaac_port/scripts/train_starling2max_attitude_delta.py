"""Train/evaluate the Starling 2 Max attitude-delta task with RL-Games."""
...
parser.add_argument(
    "--config",
    type=Path,
    default=ROOT / "config/rl_games_starling2max_attitude_delta.yaml",
)
...
env_cfg = Starling2MaxAttitudeDeltaEnvCfg()
env_cfg.scene.num_envs = args.num_envs
env_cfg.sim.device = args.device
env = gym.make(STARLING2MAX_ATTITUDE_DELTA_TASK_ID, cfg=env_cfg)
...
# rl_games asserts batch_size (num_envs * horizon_length) is divisible by
# minibatch_size. A small --num-envs (e.g. 16 for GUI viewing) makes the batch
# smaller than the configured minibatch, so fall back to a single full-batch
# minibatch whenever the configured value would not divide the batch.
_rlg_cfg = agent_cfg["params"]["config"]
_rlg_batch = args.num_envs * _rlg_cfg["horizon_length"]
if _rlg_batch % _rlg_cfg["minibatch_size"] != 0:
    _rlg_cfg["minibatch_size"] = _rlg_batch
