"""Train/evaluate the Starling 2 Max depth navigation task with RL-Games.

The Deep Collision Encoder is frozen and lives inside the environment, so the
81-D observation already contains the 64-D depth latent by the time rl_games
sees it.  Only the MLP+GRU actor-critic is trained.
"""
...
parser.add_argument(
    ...
    default="follow",
    help="follow: chase camera on the drone (default). world: static overview of env 0.",
)
parser.add_argument(
    "--view-env",
    type=int,
...
parser.add_argument(
    "--config",
    type=Path,
    default=ROOT / "config/rl_games_starling2max_navigation.yaml",
)
...
if args.view == "world":
    # Static overview of the selected environment's obstacle box.
    env_cfg.follow_viewer = False
    env_cfg.viewer.origin_type = "env"
    env_cfg.viewer.eye = (-8.0, -14.0, 8.0)
    env_cfg.viewer.lookat = (4.0, 0.0, 0.0)
env = gym.make(STARLING2MAX_NAVIGATION_TASK_ID, cfg=env_cfg)
